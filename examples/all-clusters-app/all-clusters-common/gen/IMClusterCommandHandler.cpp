...
// TODO(#5098) We should pass the Command Object and EndpointId to the cluster callbacks.
        emberAfGeneralCommissioningClusterSetFabricCallback(const_cast<uint8_t*>(fabricId), const_cast<uint8_t*>(fabricSecret), breadcrumb, timeoutMs);
            break;
        }
        default: {
            // Unrecognized command ID, error status will apply.
...
while ((TLVError = dataTlv.Next()) == CHIP_NO_ERROR)
{
  ...
  {
    case 0:
      TLVError = dataTlv.Get(daysToReturn);
      break;
    case 1:
      TLVError = dataTlv.Get(modeToReturn);
      break;
    default:
      // Unsupported tag, ignore it.
      ChipLogProgress(Zcl, "Unknown TLV tag during processing.");
...
// TODO(#5098) We should pass the Command Object and EndpointId to the cluster callbacks.
        emberAfThermostatClusterGetWeeklyScheduleCallback(daysToReturn, modeToReturn);
            break;
        }
...
while ((TLVError = dataTlv.Next()) == CHIP_NO_ERROR)
{
  ...
  {
    case 0:
      TLVError = dataTlv.Get(numberOfTransitionsForSequence);
      break;
    case 1:
      TLVError = dataTlv.Get(dayOfWeekForSequence);
      break;
    case 2:
      TLVError = dataTlv.Get(modeForSequence);
      break;
    case 3:
      // Just for compatibility, we will add array type support in IM later.
      TLVError = dataTlv.GetDataPtr(const_cast<const uint8_t *&>(payload));
      break;
    default:
      // Unsupported tag, ignore it.
      ChipLogProgress(Zcl, "Unknown TLV tag during processing.");
...
// TODO(#5098) We should pass the Command Object and EndpointId to the cluster callbacks.
        emberAfThermostatClusterSetWeeklyScheduleCallback(numberOfTransitionsForSequence, dayOfWeekForSequence, modeForSequence, payload);
            break;
        }
...
while ((TLVError = dataTlv.Next()) == CHIP_NO_ERROR)
{
  ...
  {
    case 0:
      TLVError = dataTlv.Get(mode);
      break;
    case 1:
      TLVError = dataTlv.Get(amount);
      break;
    default:
      // Unsupported tag, ignore it.
      ChipLogProgress(Zcl, "Unknown TLV tag during processing.");
...
// TODO(#5098) We should pass the Command Object and EndpointId to the cluster callbacks.
        emberAfThermostatClusterSetpointRaiseLowerCallback(mode, amount);
            break;
        }
        default: {
            // Unrecognized command ID, error status will apply.
            // TODO: Encode response for command not found
            ChipLogError(Zcl, "Unknown command %" PRIx16 " for cluster %" PRIx16, commandId, ZCL_THERMOSTAT_CLUSTER_ID);
            break;
        }
        }
    }
